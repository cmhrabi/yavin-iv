...
| 2 | [`03-runs-api-and-db-wiring.md`](./03-runs-api-and-db-wiring.md) | Real `/api/runs` (POST/GET, `[id]` GET/PATCH, `[id]/gates/[kind]` POST), full WS worker/client switch wired to the run state machine, per-user authz, `useRunSubscription` hook for live dashboard updates, `scripts/stub-worker.ts` demo, vitest happy-path. `MOCK_RUNS` retired from the live data path. |
